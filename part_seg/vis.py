...
if __name__ == "__main__":
    ...
    # cmap = np.array([cmap(i) for i in range(100)])[:,:3]
    
    np.random.seed(100)
    
    cmap = (np.random.rand(100,3))*255
    # print(cmap)
    ...
    for i in range(len(TEST_DATASET)):
        ...
        # print(ps.shape,'------------')
        input = np.concatenate((ps,normal), axis=1)
        sess, ops = get_model(batch_size=1, num_point=ps.shape[0])
        segp = inference(sess, ops, np.expand_dims(input,0), batch_size=1) 
        segp = segp.squeeze()
